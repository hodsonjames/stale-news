...
def famaMacBethRegression8Parallel(dates, firms, mdatabase, pdatabase1, pdatabase2):
    """
    Average coefficients for daily cross-sectional regression over dates given
    dates: list of days in order from starting date to ending date
    firms: list of tickers of interest
    ...
    for i in range(len(dates) - 1):
        ...
        lists = {'absAbnRet': [], 'AbnPctOld': [], 'Stories': [], 'AbnStories': [], 'Terms': [], 'MCap': [],
                 'BM': [], 'AbnRet': [], 'AbnVol': [], 'AbnVolitility': [], 'Illiq': []}
        # compute first firm with data independently for significant cached value speedup
        firm_ind = 0
        for ind in range(len(firms)):
            firm_ind += 1
            abn_ret = u.abnormalReturnDate(firms[ind], dates[i + 1], pdatabase1, False)
            if abn_ret == -1:
                continue
            abn_pct_old = u.abnormalPercentageOld(firms[ind], dates[i], mdatabase)
            if abn_pct_old == -1:
                continue
            x = u.generateXList(firms[ind], dates[i], mdatabase, pdatabase1, pdatabase2, False)
            if not x:
                continue
            lists['absAbnRet'].append(abs(abn_ret))
            lists['AbnPctOld'].append(abn_pct_old)
            ...
            # one is done
            break
        # set up for parallel (abn_ret, abn_pct_old, x) "data point" computations
        dpoint_queue = queue.Queue()  # thread safe
        thread_list = []
        # use remaining firms
        for ind in range(firm_ind, len(firms)):
            thread = threading.Thread(target=lambda q, arg1, arg2, arg3, arg4, arg5, arg6: q.put(
                [u.abnormalReturnDate(arg1, arg3, arg5, False), u.abnormalPercentageOld(arg1, arg2, arg4),
                 u.generateXList(arg1, arg2, arg4, arg5, arg6, False)]),
                                      args=(dpoint_queue, firms[ind], dates[i], dates[i + 1], mdatabase,
                                            pdatabase1, pdatabase2))
            thread.start()
        ...
        while not dpoint_queue.empty():
            ...
            abn_ret = data_point[0]
            abn_pct_old = data_point[1]
            x = data_point[2]
            # skip firms where no data is available on date
            if abn_ret == -1 or abn_pct_old == -1 or not x:
                continue
            lists['absAbnRet'].append(abs(abn_ret))
            lists['AbnPctOld'].append(abn_pct_old)
            lists['Stories'].append(x[0])
